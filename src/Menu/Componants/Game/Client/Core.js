...
(function() {
        ...
        switch (packetId) {
            ...
            case 0x63: // chat message
                ...
                var server = !!(flags & 0x80),
                    ...
                    const verifiedlogo = "<i class='material-icons Verified'>check_circle</i>"


                if (server && name !== "SERVER") name = "[SERVER] " + name;
                if (admin) name = verifiedlogo + " [Developer] " + name;
                if (mod) name = "[ADMIN] " + name;
    ...
    function drawLeaderboard() {
        ...
        for (let i = 0; i < leaderboard.items.length; i++) {
            ...
            if (leaderboard.items[i].me == true) {
                $("#lb_detail").append(`<div style="color:#faa">${last}.${leaderboard.items[i].name.split("&&&")[0]}</div>`);
            } else {
                $("#lb_detail").append(`<div style="color:#${leaderboard.items[i].name.split("&&&")[1]}">${last}.${leaderboard.items[i].name.split("&&&")[0]}</div>`);
            }
        }
    }
    function drawGrid() {
        /*var stage = new PIXI.Container()
        var gridground = PIXI.Sprite.fromImage('./assets/img/backgroundwhite.png')



        stage.addChild(gridground);*/
    }
    function drawBackgroundSectors() {
        ...
        mainCtx.font = (w / 3 | 0) + "px Cantarell";
    ...
    function drawMinimap() {
        ...
        mainCtx.textBaseline = "middle";
        mainCtx.textAlign = "center";
        mainCtx.font = sectorNameSize + "px Cantarell";
        ...
        if (cell !== null) {
            ...
            mainCtx.font = textSize + "px Cantarell";
    ...
    function drawGame() {
        ...
        $('.stats-mass').text(!isNaN(stats.score) ? ('Mass: ' + (stats.score) + '') : 'Mass: 0');
    ...
    Cell.prototype = {
        ...
        update: function(relativeTime) {
            ...
            this.drawNameSize = ~~(~~(Math.max(~~(0.5 * this.s), 54)) / 3) * 3;
        ...
        drawShape: function(ctx) {
            ...
            if (this.destroyed)
                ctx.globalAlpha = Math.max(1 - Date.now() + this.dead, 0) / 1;
            else ctx.globalAlpha = Math.min(Date.now() - this.born, 1) / 1;
        ...
        drawText: function(ctx) {
            ...
            var mass = (~~(this.s * this.s / 100));
            var y = this.y;
            if (this.name && settings.showNames) {
                drawText(ctx, false, this.x, this.y+(this.s / 80), this.nameSize, this.drawNameSize, this.name);
                y += Math.max(this.s / 1.5, this.nameSize / 5.5);
            }
            if (settings.showMass && (cells.mine.indexOf(this.id) !== -1 || cells.mine.length === 0)) {
                var Mass = mass.toString();
                drawText(ctx, true, this.x, y, this.nameSize / 6, this.drawNameSize / 1, settings.shortMass ? prettyMass(Mass) : Mass);
            }
        }
    };
    ...
    function init() {
        ...
        byId("spectatebtn").addEventListener("click", function() {
            if (settings.skin) {
                spectate();
            } else {
                spectate();
        ...
        if (args && (div = regex.exec(args.slice(1)))) {
          ...
          .then(data => {
            window.setserver(data[0].server);
          });
        };
        drawGame();
